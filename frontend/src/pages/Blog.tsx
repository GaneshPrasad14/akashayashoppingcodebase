      ...
      <div className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogs.map((post, index) => (
            ...
            >
              <Link to={`/blog/${post.slug || post._id}`} className="block aspect-video overflow-hidden">
                <img
                  src={post.image ? `${(import.meta.env.VITE_API_BASE_URL || "http://localhost:5035/api").replace('/api', '')}${post.image}` : "/placeholder.svg"}
                  alt={post.title}
